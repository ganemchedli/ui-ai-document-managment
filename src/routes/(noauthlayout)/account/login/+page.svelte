<script>
	...
	let username = 'admin@themesdesign.com';
	...
	function login() {
		if (username === 'admin@themesdesign.com' && password === '12345678') {
			const authuser = {
				username: username,
				password: password
			};
...
<div class="relative">
	...
	>
		<div class="!px-10 !py-12 card-body">
			...
			<form class="mt-10" id="signInForm" on:submit|preventDefault={() => login()}>
				<div
					class="{!validFail
						? 'hidden '
						: ''} px-4 py-3 text-sm text-red-500 border border-transparent rounded-md bg-red-50 dark:bg-red-400/20 mb-4"
				>
					Invalid credentials.
				</div>
				<div
					class="{!validSuccess
						? 'hidden '
						: ''} px-4 py-3 mb-3 text-sm text-green-500 border border-green-200 rounded-md bg-green-50 dark:bg-green-400/20 dark:border-green-500/50"
					id="successAlert"
				>
